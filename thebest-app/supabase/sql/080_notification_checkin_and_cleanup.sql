-- Refine the operational feed: remove noisy walk-in-created alerts and add
-- one notification when an appointment is actually checked in.
--
...
--   appointment_checked_in   - a customer appointment was checked in
--   online_payment_received  - an online payment landed as a paid transaction
--   payment_received         - any other successful payment
...
-- The removed category is notification-only history; deleting it does not
-- touch appointments, walk-ins, bills, or customers.
delete from public.notifications where type = 'walk_in_created';
...
after update of status, payment_status, actual_started_at on public.appointments
