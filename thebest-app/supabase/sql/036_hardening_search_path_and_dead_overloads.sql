-- Hardening cleanup (advisor 0011), no behavior change.
--
-- Pin `search_path = public` on the remaining CSP helper/trigger functions the
-- Supabase linter flagged as "Function Search Path Mutable". Bodies are
-- unchanged — only the search_path setting is added, so this cannot affect
-- scheduling behavior.
--
-- (The dead 8-arg overloads of create/update_appointment_group_with_csp,
-- from before p_type existed, are intentionally NOT dropped here — that
-- needs separate explicit sign-off. See 037_drop_dead_group_overloads.sql.)
