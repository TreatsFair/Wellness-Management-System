-- 131: recommend the earliest available start, not only gap-filling ones.
--
-- Best Fit candidates were generated exclusively from the edges of existing
-- bookings, so they answered "where does this slot in without leaving a gap?"
-- and never "how soon can this guest start?". On a quiet day that produced no
-- Best Fit at all, and on a busy one it could recommend 8:40 PM while an
-- earlier opening went unranked in the plain grid.
--
-- A third candidate kind, 'earliest', is emitted for the soonest bookable
-- moment: the next 5-minute mark from now for today, or opening time for a
-- future date. It is ranked with Best Fit (see CounterCapacitySlot.isBestFit)
-- but carries its own kind so the UI can say why it is being recommended.
-- Kind precedence when several candidates land on the same instant:
-- earliest > best_fit > standard.
--
-- Everything from 129 is retained: the plain grid stays anchored to the clock
-- hour, plus opening time when that is off the hour.
