-- Atomic appointment + transaction writes.
--
-- Problem: the app previously wrote an appointment (or updated one) and then, as a
-- SEPARATE client call, inserted the transaction row. If the second call failed
-- (network blip, app killed, etc.) the appointment was left occupying a resource
-- slot with no matching sale, and the client couldn't clean it up itself because
-- appointment DELETE is admin-only. Postgres functions execute inside a single
-- transaction, so wrapping both writes in one function makes the whole payment
-- atomic: either the appointment write + transaction both land, or neither does.
--
-- This also moves commission calculation server-side (public.csp_commission_for_items)
-- instead of trusting client-submitted commission amounts for a financial write —
-- the rates already live entirely in the DB (services.therapist_commission /
-- counter_commission, with per-therapist overrides in therapists.service_commissions),
-- so there is no reason for the client to compute or transmit them.
...
-- Walk-in checkout, single pax: create the appointment, mark it in progress, and
-- record the sale in one atomic write.
...
-- Walk-in checkout, group pax: create the appointment group and record one sale
-- covering every pax, atomically. Each pax may have a different therapist, so
-- therapist commission is summed per-allocation.
...
-- Pre-scheduled appointment arrives and pays: update the existing appointment to
-- in_progress and record the sale atomically. Re-validates resource availability
-- (via the existing overlap trigger, fired by the UPDATE) before the sale lands.
...
-- Pre-scheduled group appointment arrives and pays: update every appointment in
-- the group to in_progress and record ONE sale covering the group, atomically.
-- p_per_appointment_updates is a jsonb object keyed by appointment id (text),
-- each value optionally carrying booked_date/booked_start_time/booked_end_time/
-- booked_start_at/booked_end_at overrides for that specific appointment.
