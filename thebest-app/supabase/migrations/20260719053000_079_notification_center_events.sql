-- Complete operational notification feed: online bookings, walk-ins,
-- payments, cancellations, voids, refunds, and review alerts.
--
...
--   walk_in_created          - a new staff-created walk-in appointment
...
begin
  if tg_op = 'INSERT' then
    if lower(coalesce(new.type::text, '')) in ('walkin', 'walk_in', 'walk-in') then
      v_type := 'walk_in_created';
      v_title := 'New walk-in';
    else
      return new;
    end if;
