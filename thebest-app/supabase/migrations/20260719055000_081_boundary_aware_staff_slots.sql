-- Boundary-aware staff slot suggestions.
--
-- The existing 078 function ranks a fixed interval grid. That means an exact
-- resource boundary such as 19:33 (or 19:38 after a five-minute cleanup)
-- cannot be suggested unless it happens to land on the grid. This overload:
--
-- 1. keeps the clean outlet interval as one source of candidates;
-- 2. adds real therapist/room/hold/leave/shift boundaries;
-- 3. validates the proposed service cleanup buffer as reserved time;
-- 4. returns the adjacent therapist booking boundaries used by the UI; and
-- 5. exposes the selected therapist's booked/unavailable periods separately.
--
-- The existing five-argument function is intentionally retained for older
-- clients. Flutter calls this six-argument overload after this migration.
...
begin
  ...
  -- Staff suggestions intentionally use a denser 10-minute convenience grid.
  -- The public booking interval is a customer-facing presentation choice and
  -- must not make the internal staff scheduler discard useful times.
  select
...
drop function if exists public.get_staff_booking_schedule_context(
  date, uuid, uuid
);

create function public.get_staff_booking_schedule_context(
  p_date date,
  p_therapist_id uuid,
...
revoke all on function public.get_staff_booking_schedule_context(
  date, uuid, uuid
) from public, anon;
grant execute on function public.get_staff_booking_schedule_context(
  date, uuid, uuid
) to authenticated;
